...
- Sub-agents as prompt files under `prompts/` (no separate skills): `triage` (eligibility + selection), `explore` (autonomous requirements gathering → discovery output), `implement` (delegates to `opsx:apply`, watches CI), `review` (delegates to `superpowers:requesting-code-review`, scope filter)
