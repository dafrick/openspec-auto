...
- [ ] 1.3a Add test: `--reviewer <handle>` alone (no `--branch`) activates non-interactive mode and uses inferred branch
- [ ] 1.3b Add test: `--branch <name>` alone (no `--reviewer`) activates non-interactive mode and uses inferred reviewer
...
- [ ] 2.3 When non-interactive mode is active (any of `--yes`, `--reviewer`, or `--branch` supplied): resolve reviewer from explicit flag or `inferReviewer()`; resolve branch from explicit flag or `inferDefaultBranch()`; skip both `await input(...)` calls entirely
