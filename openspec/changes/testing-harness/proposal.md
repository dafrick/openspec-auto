...
- Add gitleaks for secrets scanning as a blocking CI job
- Add 6 skill-specific linters as experimental (non-blocking) CI jobs
...
- `core-linting`: TypeScript type checking (tsc), formatting/lint (Biome), Markdown structural linting (markdownlint-cli2), and secrets scanning (gitleaks) — all required/blocking CI jobs
- `skill-linting`: Suite of 6 experimental skill-specific linters run as individual non-blocking CI jobs to evaluate quality and spec compliance of skill files
