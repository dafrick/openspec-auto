...
## Purpose
TBD - created by archiving change tighten-triage-red-flags. Update Purpose after archive.
## Requirements
...
The triage sub-agent SHALL reject a new issue (return `NO_ELIGIBLE`) only when at least one of the following red flags is directly observable from the issue surface:
...
- **THEN** triage SHALL return `NO_ELIGIBLE` and name the "No observable ask" red flag

#### Scenario: Maintainer-confirmed duplicate rejected
- **WHEN** a maintainer comment says "duplicate of #N" or equivalent
- **THEN** triage SHALL return `NO_ELIGIBLE` and name the "Confirmed duplicate" red flag

#### Scenario: Wrong-repo issue rejected
- **WHEN** a maintainer comment confirms the issue belongs to a different repository
- **THEN** triage SHALL return `NO_ELIGIBLE` and name the "Out of scope" red flag
...
When triage returns `NO_ELIGIBLE`, the output body SHALL name the specific red flag that was observed (by name, e.g., "No observable ask", "Confirmed duplicate", "Out of scope"). A generic explanation without naming a red flag is not acceptable.
