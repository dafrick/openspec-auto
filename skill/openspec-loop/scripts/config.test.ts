...
describe(".gitignore update logic", () => {
  ...
  test("adds .openspec-loop/ to .gitignore if missing", () => {
    writeFileSync(join(tmp, ".gitignore"), ".openspec-loop.json\n");
    writeFileSync(join(tmp, ".openspec-loop.json"), JSON.stringify({ reviewer: "alice" }));
    readConfig(tmp);
    const contents = readFileSync(join(tmp, ".gitignore"), "utf8");
    assert.ok(contents.includes(".openspec-loop/"));
  });

  test("does not duplicate .openspec-loop/ if already present", () => {
    writeFileSync(join(tmp, ".gitignore"), ".openspec-loop.json\n.openspec-loop/\n");
    writeFileSync(join(tmp, ".openspec-loop.json"), JSON.stringify({ reviewer: "alice" }));
    readConfig(tmp);
    const contents = readFileSync(join(tmp, ".gitignore"), "utf8");
    assert.equal(
      (contents.match(/\.openspec-loop\//g) ?? []).length,
      1
    );
  });
