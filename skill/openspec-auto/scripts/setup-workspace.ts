 ...
 * checkout main + pull, create the branch, anchor an empty commit, push,
 * open a draft PR, then initialise and sync state.json. Returns the PR number.
 *
...
export function setupWorkspace(args: SetupArgs, cwd = process.cwd()): number {
  ...
  const prUrl = run(
    `gh pr create --draft --title ${JSON.stringify(title)} --body ${JSON.stringify(`Closes #${issue}`)} --base main`
  );
