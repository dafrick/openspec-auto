...
4. Request review: try `gh pr edit --add-reviewer <reviewer>` **best-effort** — it fails when the reviewer is also the PR author (e.g. a solo repo), so don't let that abort the run — and **always** post a PR comment `@<reviewer> — ready for review` so the handoff is visible regardless of whether the review request succeeded.
