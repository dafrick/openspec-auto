...
**First run:** invoke `opsx:propose`, suggesting the change name `{{CHANGE_NAME}}`, grounded in the discovery output. If the discovery uses new Double Diamond section names, use: Point of View as the "why", Recommendation as the design direction, Now/Next/Later as scope. If it uses old-style names (Problem, Findings, Approach, Out of scope), map them to those roles respectively. The discovery output drives the proposal — its problem framing drives the "why", its recommended approach drives the design decisions, its scope breakdown drives what the specs and tasks cover. Do not re-derive requirements from scratch; the discovery is the requirements record. Make clear that the implementation plan needs to follow test-driven development. **Capture the actual change name `opsx:propose` settled on** — don't assume it matches the suggestion. Confirm it by inspecting the directory it created under `openspec/changes/` (e.g. `git status --porcelain openspec/changes/`, `ls openspec/changes/`, or `openspec change list`). That exact name is what you report and what every later stage uses; a branch↔change-name mismatch is fine — the orchestrator carries the change name forward independently of the branch.
